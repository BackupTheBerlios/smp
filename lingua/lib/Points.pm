...
sub receive_text {
  ...
  if ($self->get_activ_points($mgr, $user_id) >= $pointscost) {
    ...
    my $dbh = $mgr->connect();
    unless ($dbh->do("LOCK TABLES $mgr->{Tables}->{USER} WRITE")) {
      warn sprintf("[Error]: Trouble locking table [%s]. Reason: [%s].",$mgr->{Tables}->{POINTS_INACTIV}, $dbh->errstr);
      $mgr->fatal_error("Database error.");
    };
...
#-----------------------------------------------------------------------------#
# CALL: $self->cost_trans($mgr, $words).                                      #
#                                                                             #
#       $mgr      = manager object.                                           #
